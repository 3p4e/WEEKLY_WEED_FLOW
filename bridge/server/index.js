...
const api = createApi({ config, store, vault, sessions, tasks, publicDir });
...
server.listen(config.PORT, config.HOST, () => {
  ...
  if (config.HOST !== '127.0.0.1' && config.HOST !== 'localhost') {
    console.warn('[bridge] WARNING: bound to a non-loopback address. This app hands out shells — put it behind TLS and keep the token secret.');
  }
});

for (const sig of ['SIGINT', 'SIGTERM']) {
  process.on(sig, () => { console.log(`[bridge] ${sig}: shutting down sessions`); sessions.shutdown(); server.close(() => process.exit(0)); setTimeout(() => process.exit(0), 2000).unref(); });
}
