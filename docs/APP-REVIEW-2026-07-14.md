...
| # | Finding | Status |
|---|---------|--------|
| **M1** | **Dept managers can read org-wide task content via `/audit`.** Audit rows carry the full `old_values`/`new_values` task JSON and are RLS-scoped by `app.is_elevated()` (org-wide), **not** department-filtered — the one place the department boundary leaks. | **DEFERRED** — add a dept predicate for dept-scoped roles on the `/audit` list path. Backend change → needs go-ahead. |
| **M2** | **AI corpus context isn't dept-scoped.** A department manager's AI answer is grounded in *all* departments' tasks, not just their own. | **DEFERRED** — scope the retrieval corpus by `_effective_dept_id` for dept-scoped roles. Backend change → needs go-ahead. |
...
| L3 | Unbounded `recurrence.interval` → a 500 that reverts a completion. | **DEFERRED** — bound the value in `_check_days`/recurrence validation (backend). |
| L4 | Unescaped task-id in a citation `href`. | **DEFERRED** — route through `GF.esc` (frontend, low-risk). |
| L5 | Unbounded `tags`. | **DEFERRED** — length/count bound (backend). |
...
## 4. Calibration — why the split

The right posture immediately before an executive demo: **apply the safe,
demo-relevant fixes now** (all frontend, one deploy, low risk — including the
two new-code bugs L1/L2 introduced by the Mass Weed work), and get **explicit
owner go-ahead before touching the audit trigger and backend on production**,
since H1/M1/M2 change the integrity subsystem and are not demo-blockers.

Recommended next batch, once approved (all backend/schema, one coordinated
deploy + prod migration window):

1. **H1** — `pg_advisory_xact_lock` in `fn_audit_row` (serialize the tail read).
2. **M1** — department predicate on `/audit` for dept-scoped roles.
3. **M2** — dept-scope the AI retrieval corpus.
4. **L3 / L4 / L5** — bound `recurrence.interval`, escape the citation href,
   bound `tags`.
...
client_id so offsite backups don't stop in 2026.
