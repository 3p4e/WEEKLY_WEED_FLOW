...
| 2 | **Agent sprawl / auto-spawner (107 agents)** | Med→High | Deletes are irreversible | Doubling since the plan suggests something spawns agents. **Investigate first** (list agents by name/created-at, find the spawn pattern / the tool or agent that creates others) before deleting anything — some of the 107 are the DocEngine `gf_*` fleet + the weekly-AI agents and must NOT be removed. Deleting a live/in-use agent breaks its caller. |
