...
rows = [
 ...
 ("OOS", "A result is flagged only when it provably exceeds the global criterion in the Parameters sheet. ND, BLQ, <LOQ, 'absent', 'Одговара' pass. Microbial counts use the Ph. Eur. 5.1.4 maximum acceptable count (2 × the stated limit), the same rule the owner's Quality Desk applies. Total Δ⁹-THC has no global criterion (per target grade) and is never flagged."),
