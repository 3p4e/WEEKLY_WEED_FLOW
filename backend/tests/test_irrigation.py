...
async def test_read_gating_and_writer_roles(client, admin_headers):
    ...
    # record: a feed is a cultivation-crew action; QC is a different manager
    denied = await _feed(client, qc_h, room["id"], water_volume_l=12)
    assert denied.status_code == 403

    ok = await _feed(client, cu_h, room["id"], method="drip", water_volume_l=40,
...
async def test_room_is_required_and_must_exist(client, admin_headers):
    ...
    missing = await client.post("/cultivation/irrigation",
                                json={"water_volume_l": 10}, headers=cu_h)
    assert missing.status_code == 422

    # a well-formed uuid that is not a room → 422 with a named reason
    ghost = await _feed(client, cu_h, "00000000-0000-0000-0000-000000000000")
...
async def test_method_and_ranges_are_refused_with_a_named_422(client, admin_headers):
    ...
    bad_method = await _feed(client, cu_h, room["id"], method="sprinkle")
    ...
    assert (await _feed(client, cu_h, room["id"], feed_ph=15)).status_code == 422
    assert (await _feed(client, cu_h, room["id"], runoff_ph=-1)).status_code == 422
    assert (await _feed(client, cu_h, room["id"], water_volume_l=-5)).status_code == 422
    assert (await _feed(client, cu_h, room["id"], feed_ec=-0.1)).status_code == 422
...
async def test_a_null_reading_is_not_zero(client, admin_headers):
    ...
    r = await _feed(client, cu_h, room["id"], water_volume_l=25)
    assert r.status_code == 201, r.text
    body = r.json()
...
async def test_defaults_to_today_at_the_site(client, admin_headers):
    ...
    r = await _feed(client, cu_h, room["id"], water_volume_l=10)
    assert r.status_code == 201, r.text
...
async def test_batch_scope_is_optional_and_filters_the_list(client, admin_headers):
    ...
    scoped = await _feed(client, cu_h, room["id"], batch_id=b["id"], water_volume_l=30)
    assert scoped.status_code == 201, scoped.text
    roomonly = await _feed(client, cu_h, room["id"], water_volume_l=5)
