"""users-DB app.fn_audit_row(): pin the function's TimeZone to UTC (H2)
...
The body is otherwise byte-identical to 0008's, so the CI schema-diff (pg_dump of
the alembic-built DB vs `schema.users.sql`) stays clean. The tasks DB carries its
own copy of this trigger; tasks-0050 makes the identical change there.

The users chain is CLEAN (269 rows, 0 breaks) — no session ever wrote to it under
a non-UTC zone. This migration is applied here anyway because the two copies of
the trigger must not diverge: a fix present in one database and absent in the
other is the kind of asymmetry that gets discovered years later by the database
that was not fixed.
"""
...
# Everything else must stay byte-identical to schema.users.sql's prosrc or the CI
# schema-diff fails.
