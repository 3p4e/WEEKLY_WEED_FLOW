...
- **Context-Aware Replacement**: Detect document context and fill accordingly
  - `Quality Manager` in approval blocks → get from personnel role
  - `Quality Assurance` → infer from document category
  - `Secure Storage Vault (Room 09)` → use from facility storage config
- **Smart Defaults**: Pre-fill common placeholders
  - `` → "7 years" (GMP standard)
  - `[FREQUENCY - To be defined in procedure]` → "Daily"
  - `Primary` → "Yes/No" based on context
- **Conditional Logic**: Handle IF/THEN placeholders
  - `[IF_IN_HOUSE_LAB]...content...End of conditional section`
  - `[IF_CONTRACT_LAB]...content...End of conditional section`
